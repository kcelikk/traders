...
golden-orders-diff:      # emir düzeyi fark tablosu (re-baseline raporu için)
	$(PY) -m scripts.golden_orders --diff tests/golden/orders_baseline.json

golden-orders-baseline:  # emir baseline'ını yenile (yalnız ADR'li re-baseline'da)
	$(PY) -m scripts.golden_orders --out tests/golden/orders_baseline.json
...
parity:                  # Gate 5 çıkış koşulu: plugin yolu gömülü mantıkla bit-eşit mi
	$(PY) -m scripts.parity_check $(if $(OUT),--out $(OUT),)
...
test-determinism:        # Rule Zero: saflık + iki process bit-eşitliği + sabitlenmiş golden hash
	$(PYTEST) -q tests/test_replay_determinism.py tests/test_purity.py tests/test_golden_replay.py tests/test_golden_orders.py

bench-transport:         # Gate 2.1: legacy vs kalıcı bağlantı RTT'si (testnet'e imzalı OKUMA atar, emir yok)
	PYTHONPATH=. $(PY) -m scripts.bench_transport --n $(or $(N),30) $(if $(OUT),--out $(OUT),)
